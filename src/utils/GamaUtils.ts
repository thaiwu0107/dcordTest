...
import { IString } from '../models/IGamaContext';
...
export default class GamaUtils {
    ...
    public static MemberIdToMemberSerialAndSectionSerial(memberId: IString | string) {
        if (_.size(memberId) !== 8) {
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL, 'MemberId Size Must to be 8');
        }
    ...
    public static DBTimeFormat(time: Date | string) {
        return moment(time).format(GamaConstant.DB_TIME_FORMAT);
    }
    public static DateTimeFormat(time: Date | string) {
        return moment(time).format(GamaConstant.DATE_TIME_FORMAT);
    }
    public static PDFTimeFormat(time: Date | string | number) {
        return moment(time).format(GamaConstant.PDF_TIME_FORMAT);
    }
    public static BirthdayTimeFormat(time: Date | string) {
        return moment(time).format(GamaConstant.BIRTHDAY_TIME_FORMAT);
    }
    public static PDFHoursTimeFormat(time: Date | string) {
        return moment(time).format(GamaConstant.PDF_HOURS_TIME_FORMAT);
    }
    ...
    public static GetCheckSum(sASCIICardSerial: string) {
        const stringArray = _.map(sASCIICardSerial, (data) => {
            ...
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL, 'Have To All Number In Strings');
        });
        if (_.size(stringArray) !== 12) {
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL, 'Have To Equal 12 size');
        }
    ...
    public static EncryptCard(card: string) {
        const stringArray = _.map(card, (data) => {
            ...
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL, `Have To All Number In Strings(${card})`);
        });
        if (_.size(stringArray) !== 8) {
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL, 'Have To Equal 8 size');
        }
        const addMagic = _.toString(_.toInteger(card) + GamaConstant.OPCARD_MAGIC_NUMBER);
        ...
        return this.JoinArray(
            _.map(_.chunk(magicArray, 2), (magicPair) => {
                const magicPairNumber = _.toInteger(this.JoinArray(magicPair)) + GamaConstant.OPCARD_MAGIC_PAIR_NUMBER;
    ...
    public static DecryptCard(card: string) {
        ...
        ])) - GamaConstant.OPCARD_MAGIC_NUMBER), 8, '0');
    }
    ...
    public static getShortCardSerialPrintFormat(card: string, algorit: boolean = false) {
        ...
        return GamaConstant.OPCARD_PRINTER_HEAD_EMPTY + encryptCard + GamaConstant.OPCARD_PRINTER_SHORTEND;
    }
    public static getLongCardSerialPrintFormat(card: string, algorit: boolean = false) {
        const encryptCard = algorit ? this.EncryptAlgorithmCard(card) : this.EncryptOriginalCard(card);
        const magicString =
            GamaConstant.OPCARD_PRINTER_HEAD_EMPTY + encryptCard + GamaConstant.OPCARD_LONG_PRINTER_MAGICSTRING1;
        const magneticStripeCode = GamaConstant.OPCARD_LONG_PRINTER_MAGICSTRING2 + encryptCard;
        const iChecksum = this.CardSerialCheckSum(magneticStripeCode);
        if (iChecksum <= 9) {
            return magicString + magneticStripeCode + _.toString(iChecksum) + GamaConstant.OPCARD_PRINTER_AFTER9END;
        } else if (iChecksum === 10) {
            return magicString + magneticStripeCode + GamaConstant.OPCARD_PRINTER_BE10END;
        } else if (iChecksum === 11) {
            return magicString + magneticStripeCode + GamaConstant.OPCARD_PRINTER_BE11END;
        } else if (iChecksum === 12) {
            return magicString + magneticStripeCode + GamaConstant.OPCARD_PRINTER_BE12END;
        } else if (iChecksum === 13) {
            return magicString + magneticStripeCode + GamaConstant.OPCARD_PRINTER_BE13END;
        } else if (iChecksum === 14) {
            return magicString + magneticStripeCode + GamaConstant.OPCARD_PRINTER_BE14END;
        } else {
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL, 'Long CardSerial Print Format Fail');
        }
    ...
    public static deeplyToCamelCase(obj) {
        ...
        return _.reduce(obj, (r, v, k) => {
            return {
                ...r,
                [_.lowerFirst(k)]: GamaUtils.deeplyToCamelCase(v)
            };
        }, {});
    }
    /**
    ...
    public static deeplyToUpperCamelCase(obj) {
        ...
        return _.reduce(obj, (r, v, k) => {
            return {
                ...r,
                [_.upperFirst(k)]: GamaUtils.deeplyToUpperCamelCase(v)
            };
        }, {});
    }
    /**
     ...
     * @memberOf Utils
     */
    public static Encryption_AES_ECB_128(data: string, ecbKey: string = GamaConstant.AES_128_ECB_KEY) {
        const cipherChunks: any[] = [];
        const cipher = crypto.createCipheriv(
            GamaConstant.AES_CIPHER,
            new Buffer(ecbKey),
            GamaConstant.AES_CIPHER_IV);
        cipher.setAutoPadding(true);
        cipherChunks.push(
            cipher.update(data, GamaConstant.AES_CLEAR_ENCODING, GamaConstant.AES_CIPHER_ENCODING)
        );
        cipherChunks.push(
            cipher.final(GamaConstant.AES_CIPHER_ENCODING)
        );
     ...
     * @memberOf Utils
     */
    public static Decryption_AES_ECB_128(data: string, ecbKey: string = GamaConstant.AES_128_ECB_KEY) {
        const cipherChunks: any[] = [];
        ...
        cipherChunks.push(
            decipher.update(data, GamaConstant.AES_CIPHER_ENCODING, GamaConstant.AES_CLEAR_ENCODING)
        );
        cipherChunks.push(
            decipher.final(GamaConstant.AES_CLEAR_ENCODING)
        );
    ...
    public static checkNumber(value: number) {
        if (!Number.isInteger(value)) {
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL, 'checkNumber() must have to integer');
        }
        return (value & 1) === 0;
    }
    /**
    ...
    public static TwoArrayToObj(keys: string | string[], values: any | any[]): any {
        if (_.size(keys) !== _.size(values)) {
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL,
                'GamaUtils.TwoArrayToObj() keys and values size not equal');
        }
    ...
    public static getTicketSEValidNum(ticketValidationId: number, ticketSeedSeqNum: number): string {
        if (!Number.isInteger(ticketValidationId) || !Number.isInteger(ticketSeedSeqNum)) {
            throw new LibsExceptions(GamaHttpStatusCode.STATUS_FAIL,
                'GamaUtils.geTicketSEValidNum() ticketValidationId || ticketSeedSeqNum have to be Integer');
        }
